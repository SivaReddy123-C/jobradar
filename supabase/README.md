...
2. Apply `migrations/20260913070219_hosted_discovery.sql` to the project. This migration starts new searches **disabled**. Its private `jr_discovery` schema is not exposed to PostgREST. Tables have RLS with no public policies; only the service role has access. Public RPC entry points use SECURITY INVOKER and revoke execution from PUBLIC, anon and authenticated.
