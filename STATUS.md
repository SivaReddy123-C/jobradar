...
Validation: **163 unit tests** (42 app, 109 collector, 12 extension), TypeScript checks, app/extension builds, **13 installed-extension browser scenarios**, and **6 discovery UI scenarios**. The live-trial follow-up reran the 109 collector and 42 app tests, their TypeScript checks, app build and six discovery UI scenarios; extension runner validation is retained from the preceding build. Two new regression tests cover the 30-day boundary and aging cached results without another provider request. Prior JSearch verification confirms local credential/cache files are blocked by the development server and cross-origin discovery requests are rejected. App checks cover profile/resume persistence, separate market answers, queue creation, missing-profile gates, desktop/mobile layouts and a clean browser error log. No employer received fixture data.
