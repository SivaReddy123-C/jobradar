# JobRadar — hosted discovery activation, September 13, 2026

The existing free Supabase JobRadar project is healthy. The private discovery migration and `job-discovery` Edge Function are deployed, the existing provider key is installed as a hosted secret, and new searches are **enabled**. This release connects the public app to confirmed-account sign-in, shared 24-hour caches, atomic reservations, per-user/day and global/day/rolling-month quotas, a bounded provider call, and private normalized listing storage. Seven earlier local provider requests were included in the hosted ledger.

Live activation checks: an authenticated India accountant search returned 10 records and retained four matching listings; a USA software-engineer search returned 10 and retained 10. These two searches consumed two provider credits, bringing the hosted ledger to nine. Replays, sign-in through the app, automatic restoration, explicit repeat searches, reload and sign-out used no further credits. Temporary test users and sessions were removed. Authentication, CORS, private RPC denial and browser runtime checks passed. These are listing counts, not independently verified vacancies.

Auth now redirects to the renamed `/jobradar/` site instead of the prior localhost default; existing allowed redirects were preserved. Email confirmation remains required. Confirmed-account sign-in is live-tested; delivery of verification emails to new public accounts remains unverified and is required before a general signup rollout. Operational instructions are in [supabase/README.md](supabase/README.md).

New verification: seven collector HTTP/cache tests, five browser scenarios with intercepted Auth/provider requests, and deployed SQL assertions for privileges, RLS, quota reservations, leases, TTL, cooldown and minute limits. Two simultaneous deployed reservations produced one `reserved` and one `busy`; their fixture records were removed. All 173 unit tests, package type checks/builds, 13 installed-extension scenarios, seven local discovery scenarios and five hosted UI scenarios passed in [feature-branch CI](https://github.com/SivaReddy123-C/jobradar/actions/runs/34745494030). Advisor notes and known email-delivery limits are recorded in the hosting guide.
...
Public deployment now includes the exact USA/India feed files from the release under `feed/`. It no longer depends on the raw GitHub `main` URL, which was observed returning the August 27 feed after the new interface deployed. A newly published index also replaces recently cached older market data. This release keeps the app and its job snapshot together.
...
Validation: **166 unit tests** (43 app, 111 collector, 12 extension), TypeScript checks, app/extension builds, **13 installed-extension browser scenarios**, and **7 discovery UI scenarios**. The deployment fix reran the 43 app tests and production build/TypeScript check; its added regression covers replacing a recently cached older shard when the published snapshot changes. Earlier collector/extension and browser verification remains applicable. Restoration tests cover selected role/country boundaries, missing/expired caches, credential-free reads, unchanged quotas and reload persistence without an automatic provider search. Prior JSearch verification confirms local credential/cache files are blocked by the development server and cross-origin discovery requests are rejected. App checks cover profile/resume persistence, separate market answers, queue creation, missing-profile gates, desktop/mobile layouts and a clean browser error log. No employer received fixture data.
...
Daily refresh and sponsorship schedules remain paused. The legacy account-backed feed remains outside navigation. Supabase, accounts and billing are unchanged. GitHub Pages has been updated at the user's request.
