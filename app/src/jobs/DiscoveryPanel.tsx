...
export function DiscoveryPanel({ preferences, onResult, added }: { preferences: SearchPreferences; onResult: (result: DiscoveryResult) => void; added: number }) {
  ...
  return <aside className="card discovery-panel" aria-label="Search more job sites">
    ...
    {result && <p role="status">{added} additional unique matches available · {result.requestsUsed} new requests in the last search. {result.queries.map(q => `${MARKETS[q.query.market]}: ${q.matches}/${q.returned} results match${q.cached ? " (cached)" : ""}`).join("; ")}. {result.queries.some(q => q.moreAvailable) && "The provider has more pages; this trial fetched one."}</p>}
