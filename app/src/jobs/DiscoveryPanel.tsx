...
export function DiscoveryPanel({ preferences, onResult, added }: { preferences: SearchPreferences; onResult: (result: DiscoveryResult) => void; added: number }) {
  ...
  useEffect(() => {
    let active = true;
    setRestoring(true); setRestored(false);
    void request<DiscoveryResult>({ action: "restore", preferences }).then(value => {
  ...
  return <aside className="card discovery-panel" aria-label="Search more job sites">
    ...
    <div className="discovery-controls"><label className="field">Role to search<select value={roleId} disabled={busy || restoring} onChange={e => setRoleId(e.target.value)}>{preferences.roles.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}</select></label><button className="primary" disabled={busy || restoring || !status?.configured} onClick={() => void search()}>{restoring ? "Restoring saved results…" : busy ? "Searching…" : "Find additional jobs"}</button><button disabled={busy} onClick={() => { setError(""); void request<Status>({ action: "status" }).then(setStatus).catch(e => setError(e.message)); }}>Check connection</button></div>
    <p className="hint">{preferences.markets.map(m => MARKETS[m]).join(" + ")} · One page per country · Up to {preferences.markets.length} request credits per search. Saved results restore automatically for 24 hours without API credits.</p>
    ...
    {result && <p role="status">{restored && "Saved search results restored automatically. "}{added} additional matching listings available · {result.requestsUsed} new requests{restored ? " to restore results" : " in the last search"}. {result.queries.map(q => `${q.query.role} · ${MARKETS[q.query.market]}: ${q.matches}/${q.returned} results match${q.cached ? " (cached)" : ""}`).join("; ")}. {result.queries.some(q => q.moreAvailable) && "The provider has more pages; this trial fetched one."} Exact link duplicates are removed; different sites may advertise the same opening.</p>}
